...
export default function VerificationManagement() {
  ...
  const fetchApplications = async () => {
    try {
      // Fetch fundi profiles via API
      const response = await apiClient.request('/fundi/all', { includeAuth: true });
      setApplications(response.fundis || []);
    } catch (error) {
  ...
  const handleApprove = async (app: FundiApplication) => {
    ...
    try {
      // ...existing code...
        ...
        .eq("id", app.id);
await apiClient.request(`/fundi/${app.id}/verify`, {
        method: 'PATCH',
