...
export default function FundiTracker({
  ...
  // Safety check: if no jobId, show error immediately
  if (!jobId) {
    return (
  ...
  // Safety check: if no auth token, redirect to login
  ...
  useEffect(() => {
    ...
    blockBackRef.current = status === 'searching' || status === 'requested' || status === 'matched';
        ...
        <div className="status-center">
          ...
          {['matched', 'accepted'].includes(status) && fundi && (
            <div className="found-block">
              ...
              <div className="actions large-actions" style={{ marginTop: '16px' }}>
          ...
          {['on_the_way','in_progress'].includes(status) && fundi && (
            <div className="live-block">
              ...
              <div id="tracking-map" className="map-placeholder" style={{ width: '100%', height: '280px', borderRadius: '12px', marginBottom: '16px', background: 'linear-gradient(180deg, #0b1220 0%, #071026 100%)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#93c5fd', fontSize: '14px' }}>
                Map loading...
              </div>
          ...
          {status === 'completed' && (
            <div className="complete-block">
              ...
              <div className="actions">
                <Button onClick={() => { onComplete && onComplete(); }}>Done</Button>
              </div>
            </div>
          )}
        </div>
