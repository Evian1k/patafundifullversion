#!/usr/bin/env node
import { query } from '../src/db.js';
import { hashPassword } from '../src/utils/password.js';
import { v4 as uuidv4 } from 'uuid';

async function setupAdmin() {
  const email = process.env.ADMIN_EMAIL || 'emmanuelevian@gmail.com';
  const password = process.env.ADMIN_PASSWORD || 'change-me';
  const fullName = process.env.ADMIN_FULL_NAME || 'Admin';

  try {
    console.log('🔐 Creating admin account...');
    
    // Hash password
    const passwordHash = await hashPassword(password);

    // Check if already exists
    const existing = await query('SELECT id, role FROM users WHERE email = $1', [email]);
    
    if (existing.rows.length > 0) {
      const user = existing.rows[0];
      // Promote (or re-promote) user to admin and ensure password matches the configured admin password.
      const updateResult = await query(
        `UPDATE users
         SET role = 'admin',
             password_hash = $1,
             full_name = COALESCE(NULLIF($2, ''), full_name),
             email_verified = true,
             updated_at = CURRENT_TIMESTAMP
         WHERE id = $3
         RETURNING id, email, role`,
        [passwordHash, fullName, user.id]
      );
      console.log('✅ Admin account is ready (promoted/updated)');
      console.log(`   Email: ${email}`);
      console.log(`   Password: set from ADMIN_PASSWORD`);
      console.log('   User:', updateResult.rows[0]);
      process.exit(0);
    }

    // Create admin user
    const userId = uuidv4();
    const result = await query(
      `INSERT INTO users (id, email, password_hash, full_name, role, email_verified)
       VALUES ($1, $2, $3, $4, $5, true)
       RETURNING id, email, full_name, role`,
      [userId, email, passwordHash, fullName, 'admin']
    );

    console.log('✅ Admin user created successfully!');
    console.log(`   Email: ${email}`);
    console.log(`   Password: set from ADMIN_PASSWORD`);
    console.log('   User:', result.rows[0]);
    
    process.exit(0);
  } catch (error) {
    console.error('❌ Error:', error.message);
    process.exit(1);
  }
}

setupAdmin();
